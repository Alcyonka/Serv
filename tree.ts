...
search(node:NodeTree|null, data:string|number):NodeTree|null {
    if (node === null) {
        return null;
...
/*changeNode(dataOld:string|number, dataNew:string|number){
...
delete(node:NodeTree|null,data:string|number){
    if (!node){
...
height(node:NodeTree|null):number {
...
console.log("Ищем 7");
    let found = null;
    if(tree.root) found=tree.search(tree.root, 7);
    if (found){
        console.log("Нашли");
    }else{
...
if(creater) creater.addEventListener('click', createTree);
